(() => { if (window.BEAEPAGEJS === undefined) {
          ...
          window.beaePageSetting.pageId = "6a17c472ee271acbfb1fed35";
  ...
  const formFields = data.el.querySelectorAll('.beae-form-field');
  const inputWrapper = data.el.querySelector('.beae-form-container');

  if (formFields.length === 0 || inputWrapper == null) return;

  formFields.forEach((formField) => {
    const input = formField.querySelector('input, textarea, select');
    const formHead = formField.querySelector('.beae-form-head');
    ...
    const updateFieldUI = () => {
      ...
      if (value !== '') {
        ...
        if (formHead) formHead.style.display = 'none';
      } else {
        formField.classList.remove('active');
        if (formHead) formHead.style.display = '';
      }
    };

    updateFieldUI();
    input.addEventListener('change', updateFieldUI);
